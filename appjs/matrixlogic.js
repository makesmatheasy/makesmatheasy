function creatematrix1() {
  ...
  for (var i = 0; i < document.getElementById("row1").value; i++) {
    ...
    document
      .getElementById("generatedmatrix1")
...
function creatematrix2() {
  ...
  for (var i = 0; i < document.getElementById("row2").value; i++) {
    ...
    document
      .getElementById("generatedmatrix2")
      .appendChild(document.createElement("br"));
  }
}
...
function sumofmatrix() {
  ...
  setTimeout(function () {
    ...
    for (var i = 0; i < document.getElementById("row2").value; i++) {
      summ[i] = [];
      sumexplanation +=
        "<div style='border-radius:50px;padding:20px;min-width:900px;' class='bi'>";
      for (var j = 0; j < document.getElementById("column2").value; j++) {
        sumexplanation +=
          "\\[a_{" +
          String(parseInt(i + 1)) +
          String(parseInt(j + 1)) +
          "} = " +
          String(parseInt(matrix1[i][j])) +
          "\\space and \\space b_{" +
          String(parseInt(i + 1)) +
          String(parseInt(j + 1)) +
          "} = " +
          String(parseInt(matrix2[i][j])) +
          "\\]";
        ...
        for (var ii = 0; ii < document.getElementById("row2").value; ii++) {
          for (
            var jj = 0;
            jj < document.getElementById("column2").value;
            jj++
          ) {
            if (ii == i && jj == j) {
              dp += matrix1[ii][jj] + "&";
        ...
        for (var ii = 0; ii < document.getElementById("row2").value; ii++) {
          for (
            ...
            if (ii == i && jj == j) {
              dp +=
                String(matrix1[ii][jj]) + "+" + String(matrix2[ii][jj]) + "&";
            } else {
              dp += ".&";
            }
          }
        ...
        for (var ii = 0; ii < document.getElementById("row2").value; ii++) {
          for (
            ...
            if (ii == i && jj == j) {
              dp +=
                eval(String(matrix1[ii][jj]) + "+" + String(matrix2[ii][jj])) +
                "&";
            } else {
              dp += ".&";
            }
    ...
    document.getElementById(
      "explanationmatrixresult"
    ).innerHTML = sumexplanation;
    displaymatrix(
      "Addition\\space Result",
      summ,
      "matrixresult",
      String(document.getElementById("row2").value),
      String(document.getElementById("column2").value)
    );
...
function subtractofmatrix() {
  ...
  setTimeout(function () {
    ...
    document.getElementById(
      "explanationmatrixresult"
    ).innerHTML = subbexplanation;
    displaymatrix(
      "Subtraction\\space Result",
      subb,
      "matrixresult",
      String(document.getElementById("row2").value),
      String(document.getElementById("column2").value)
...
function mulofmatrix() {
  ...
  setTimeout(function () {
    ...
    for (var i = 0; i < document.getElementById("row1").value; i++) {
      ...
      mulexplanation +=
        '<span style="font-size: 20px;"> Taking Row ' +
        String(i + 1) +
        " of Matrix 1</span>";
      ...
      for (var j = 0; j < document.getElementById("column2").value; j++) {
        mulexplanation +=
          "<div class='bi' style='border-radius:50px;padding:20px;display:table;margin:3px;width:100%;'>";
        mulexplanation +=
          '<span style="border: 3px solid var(--appblack);padding: 10px;border-radius: 30px;font-size: 20px;">Column ' +
          String(j + 1) +
          " of Matrix 2</span><br>";
        mul[i][j] = 0;
        for (var k = 0; k < document.getElementById("column1").value; k++) {
          mulexplanation +=
            ...
            String(parseInt(k + 1)) +
            "} = " +
            String(parseInt(matrix1[i][k])) +
            "\\space and \\space b_{" +
            String(parseInt(k + 1)) +
            String(parseInt(i + 1)) +
            "} = " +
            String(parseInt(matrix2[k][j])) +
            "\\]\\[\\big( " +
            String(parseInt(matrix1[i][k])) +
            " &times; " +
            String(parseInt(matrix2[k][j])) +
            " \\big) = " +
            String(parseInt(matrix1[i][k]) * parseInt(matrix2[k][j])) +
            "\\]";
          temp +=
            String(parseInt(matrix1[i][k]) * parseInt(matrix2[k][j])) + " + ";
          mul[i][j] =
            parseInt(mul[i][j]) +
            parseInt(matrix1[i][k]) * parseInt(matrix2[k][j]);
        }
        var dp = "\\[\\begin{bmatrix}";
        var count = 0;
        for (var ij of matrix1) {
          for (var ji of ij) {
            if (count == i) dp += "\\color{blue}" + ji + "&";
            else {
              dp += ji + "&";
            }
          }
    ...
    document.getElementById(
      "explanationmatrixresult"
    ).innerHTML = mulexplanation;
    renderMathInElement(document.getElementById("explanationmatrixresult"));
    displaymatrix(
      "Multiplication\\space Result",
      mul,
      "matrixresult",
      String(document.getElementById("row1").value),
      String(document.getElementById("column2").value)
    );
    // <!--            printhere-->
  }, 100);
...
function checkfunctionsmultiple() {
  ...
  if (r1 == "" || r2 == "" || c1 == "" || c2 == "") {
    ...
    document.getElementById("mmatrixerror").innerHTML =
      "<center>Please enter positive integers for the dimensions.</center>";
    removeall("generatedmatrix1");
    removeall("generatedmatrix2");
  } else {
    ...
    if (r1 == r2 && c1 == c2) {
      if (c1 != r2) {
        ...
        addop("opval", "-");
        document.getElementById("mmatrixerror").innerHTML =
          "<center>Multiplication is not Possible with these inputs</center>";
      } else if (c1 == r2) {
        addop("opval", "Select Operation");
        addop("opval", "+");
        addop("opval", "-");
      ...
      if (((c1 == r2) == r1) == c2) {
        ...
        addop("opval", "×");
        document.getElementById("mmatrixerror").innerHTML =
          "<center>Addition and Subtraction is not Possible with these inputs</center>";
        signofmatrix("×");
      } else if (c1 == r2 && r1 == c2) {
        addop("opval", "×");
        document.getElementById("mmatrixerror").innerHTML =
...
function creatematrixsingle() {
  ...
  for (var i = 0; i < document.getElementById("srow1").value; i++) {
    ...
    document
      .getElementById("generatedmatrixsingle")
      .appendChild(document.createElement("br"));
  }
}
...
function transpose() {
  ...
  setTimeout(function () {
    ...
    displaymatrix(
      "Transposed\\space Matrix",
      trans,
      "singlematrixresult",
      String(document.getElementById("scolumn1").value),
      String(document.getElementById("srow1").value)
    );
    document.getElementById("singlematrixexplanation").innerHTML =
      "&nbsp;&nbsp;Just Interchange Rows and Columns";
  }, 100);
  setTimeout(function () {
    loader("hide");
  }, 2000);
...
function rank() {
  ...
  setTimeout(function () {
    sendtomatrixsingle();
    var rankexplanation =
      "<div style='border-radius:50px;display:table;color:black;padding:20px;margin-left: auto; margin-right: auto;'>";
    var row = document.getElementById("srow1").value;
    var column = document.getElementById("scolumn1").value;
    var rank = column;
    //---print initial matrix
    rankexplanation +=
      "<div style='border:none;border-radius:30px;margin:2px'>\\[Given \\space Matrix\\]";
    ...
    for (var i = 0; i < rank && i < row; i++) {
      ...
      if (matrixsingle[i][i] != 0) {
        ...
        if (i != row - 1 && eliminateflag == 0) {
          rankexplanation +=
            "<div style='border:none;border-radius:30px;margin:2px'>\\[Eliminate \\space elements \\space in \\space the \\space " +
            (i + 1) +
            " \\space column \\space under \\space" +
            (i + 1) +
            "\\space element\\]";
        }
      } else {
        ...
        for (var j = i + 1; j < row; j++) {
          if (matrixsingle[j][i] != 0) {
            rankexplanation +=
              "<div style='border:none;border-radius:30px;margin:2px'>\\[Swap \\space the \\space " +
              (i + 1) +
              " \\space and \\space " +
              (j + 1) +
              "\\space row\\]";
    ...
    rankexplanation +=
      "\\[Count \\space number\\space of \\space non \\space zero \\space rows/columns,\\space that \\space will \\space be \\space rank.\\\\Rank\\space Of \\space Matrix=" +
      rank +
      "\\]";
    rankexplanation += "</div>";
    document.getElementById("singlematrixresult").innerHTML =
      "\\[Rank \\space Of \\space Matrix=" + rank + "\\]";
    document.getElementById(
      "singlematrixexplanation"
    ).innerHTML = rankexplanation;
  ...
  setTimeout(function () {
    ...
    for (var i = 0; i < row; i++) {
      for (var j = 0; j < col; j++) {
        ...
        var lk = cofactor[i][j];
        temp +=
          "<div style='border:3px solid black;border-radius:30px;margin:2px'>\\[Co-Factor\\]";
        temp +=
          "\\[C_{" + String(parseInt(i + 1)) + String(parseInt(j + 1)) + "}=";
        temp +=
          "(-1)^{" +
          String(parseInt(i + 1)) +
          "+" +
    ...
    for (var i = 0; i < row; i++) {
      for (var j = 0; j < col; j++) {
        ...
        var lk = minor[i][j];
        temp +=
          "<div style='border:3px solid black;border-radius:30px'>\\[Minor\\]";
        temp +=
          "\\[M_{" + String(parseInt(i + 1)) + String(parseInt(j + 1)) + "}=";
        temp += "\\begin{vmatrix}" + matrixsingle[i][j];
        temp += "\\end{vmatrix}";
        temp += "=" + lk;
        temp += "\\]</div>";
        temp +=
          "<div style='height:2px !important;margin:20px !important;'></div>";
      }
    }
    temp += "</div>";
    displaymatrix(
      "Co-Factor\\space Matrix",
      cofactor,
      "singlematrixresult",
      String(document.getElementById("scolumn1").value),
      String(document.getElementById("srow1").value)
...
function checkfunctions() {
  ...
  if (row == "" || column == "") {
    ...
    document.getElementById("smatrixerror").innerHTML =
      "<center>Please enter positive integers for the dimensions.</center>";
    removeall("generatedmatrixsingle");
  } else removeall("smatrixerror");
...
function determinant(ma) {
  ...
  if (row == 2 && column == 2) {
    ...
    lets2 +=
      ...
      "\\times" +
      ma[1][1] +
      "\\color{red}\\big)\\color{black}-\\color{red}\\big(\\color{black}" +
      ma[0][1] +
      "\\times" +
      ma[1][0] +
      "\\color{red}\\big)";
    ...
    var lets22 = "\\[";
    var ss = nerdamer(ma[0][0] * ma[1][1])
      .evaluate()
      .toString();
    var ss2 = nerdamer(ma[0][1] * ma[1][0])
      .evaluate()
      .toString();
    lets22 +=
      "\\color{red}\\big(\\color{black}" +
    ...
    for (var lk = 0; lk < column; lk++) {
      ...
      for (var i = 0; i < row; i++) {
        submatrix[l] = [];
        if (i == parseInt(firstrow[jj].slice(0, 1))) continue;
        for (var j = 0; j < column; j++) {
          if (j == parseInt(firstrow[jj].slice(1, 2))) continue;
      ...
      if (
        submatrix.length ==
        parseInt(document.getElementById("srow1").value) - 1
      ) {
        vtfinalfirst +=
          "\\color{red}" + firstrow[firstrowel] + "\\bigg(\\color{black}";
        vtfinalfirstresult +=
          "\\color{red}" + firstrow[firstrowel] + "\\bigg(\\color{black}";
        vtfinalfirstresult2 +=
          "\\color{red}" + firstrow[firstrowel] + "\\bigg(\\color{black}";
        ...
        vtfinalfirstresult +=
          "\\big(" +
          eval(submatrix[0][0] * submatrix[1][1]) +
          "\\big)-\\big(" +
          eval(submatrix[0][1] * submatrix[1][0]) +
          "\\big)";
        vtfinalfirstresult2 += eval(
          eval(submatrix[0][0] * submatrix[1][1]) -
            eval(submatrix[0][1] * submatrix[1][0])
        );
        vtfinalfirstresult2final += eval(
          eval(submatrix[0][0] * submatrix[1][1]) -
            eval(submatrix[0][1] * submatrix[1][0])
        );
      }
      ...
      if (submatrix.length > 2) {
        ...
        vtofsub +=
          "\\color{red}" + firstrow[firstrowel] + "\\bigg(\\color{black}";
        vtfinalofsub +=
          "\\color{red}" + firstrow[firstrowel] + "\\left(\\color{black}";
        vtfinalofsubresult +=
          "\\color{red}" + firstrow[firstrowel] + "\\left(\\color{black}";
        vtfinalofsubresult2 +=
          "\\color{red}" + firstrow[firstrowel] + "\\left(\\color{black}";
        ...
        for (var lkofsub = 0; lkofsub < colofsub; lkofsub++) {
          ...
          vtfinalresult3 +=
            "\\color{blue}" +
            firstrowofsub[firstrowelofsub][0] +
            "\\big(\\color{black}";
          ...
          vtfinalresult +=
            "\\big(" +
            eval(submatrixofsub[0][0] * submatrixofsub[1][1]) +
            "\\big)-\\big(" +
            eval(submatrixofsub[0][1] * submatrixofsub[1][0]) +
            "\\big)\\color{blue}\\bigg)";
          var sol = eval(
            eval(submatrixofsub[0][0] * submatrixofsub[1][1]) -
              eval(submatrixofsub[0][1] * submatrixofsub[1][0])
          ...
          vtfinalresult3 +=
            eval(firstrowofsub[firstrowelofsub].slice(1) * sol) +
            "\\color{blue}\\big)";
          //			vtfinalresult3+='\\big)'
          ...
          if (submatrixofsub.length > 2) {
            ...
            for (
              var jofsubofsub = 0;
              jofsubofsub < colofsubofsub + colofsubofsub;
              jofsubofsub += 2
            ) {
              if (elofsubofsub % 2 == 0) {
                var signofsubofsub = "+";
              ...
              firstrowofsubofsub[jofsubofsub] =
                signofsubofsub + submatrixofsub[0][elofsubofsub];
            ...
            vtfinalofsub +=
              "\\color{blue}" +
              firstrowofsub[firstrowelofsub - 2] +
              "\\bigg(\\color{black}";
            ...
            for (
              ...
              for (
                var iofsubofsub = 0;
                iofsubofsub < rowofsubofsub;
                iofsubofsub++
              ) {
                submatrixofsubofsub[lofsubofsub] = [];
                if (
                  iofsubofsub ==
                  parseInt(firstrowofsubofsub[jjofsubofsub].slice(0, 1))
                )
                  continue;
                for (
                  var jofsubofsub = 0;
                  jofsubofsub < colofsubofsub;
                  jofsubofsub++
                ) {
                  if (
                    jofsubofsub ==
                    parseInt(firstrowofsubofsub[jjofsubofsub].slice(1, 2))
                  )
                    continue;
                  submatrixofsubofsub[lofsubofsub][kofsubofsub] =
                    submatrixofsub[iofsubofsub][jofsubofsub];
                  kofsubofsub = (kofsubofsub + 1) % (rowofsubofsub - 1);
                  if (kofsubofsub == 0) lofsubofsub++;
                }
              }

              vtofsub +=
                "\\color{#fb650d}" +
                firstrowofsubofsub[firstrowelofsubofsub] +
                "\\color{black}\\begin{bmatrix}";
              vtfinalofsub +=
                "\\color{#fb650d}" +
                firstrowofsubofsub[firstrowelofsubofsub] +
              ...
              vtfinalofsubresult3 +=
                "\\color{#fb650d}" +
                firstrowofsubofsub[firstrowelofsubofsub][0] +
                "\\color{black}";
              ...
              vtfinalofsub +=
                "\\big(" +
                submatrixofsubofsub[0][0] +
                "\\times" +
                submatrixofsubofsub[1][1] +
                "\\big)-\\big(" +
                submatrixofsubofsub[0][1] +
                "\\times" +
                submatrixofsubofsub[1][0] +
                "\\big)\\color{#fb650d}\\bigg)";

              vtfinalofsubresult +=
                "\\big(" +
                eval(submatrixofsubofsub[0][0] * submatrixofsubofsub[1][1]) +
                "\\big)-\\big(" +
                eval(submatrixofsubofsub[0][1] * submatrixofsubofsub[1][0]) +
                "\\big)\\color{#fb650d}\\bigg)";
              var sol = eval(
                eval(submatrixofsubofsub[0][0] * submatrixofsubofsub[1][1]) -
                  eval(submatrixofsubofsub[0][1] * submatrixofsubofsub[1][0])
              ...
              vtfinalofsubresult2 +=
                "\\color{#fb650d}\\big(\\color{black}" +
                sol +
                "\\color{#fb650d}\\big)\\color{black}";
              vtfinalofsubresult3 +=
                "\\color{#fb650d}\\big(\\color{black}" +
                eval(firstrowofsubofsub[firstrowelofsubofsub].slice(1) * sol) +
                "\\color{#fb650d}\\big)\\color{black}";
              sol2sub +=
                "(" +
                eval(firstrowofsubofsub[firstrowelofsubofsub].slice(1) * sol) +
            ...
            vtfinalofsubresult4 +=
              "\\color{blue}\\big(\\color{black}" +
              nerdamer(sol2sub).evaluate().toString() +
              "\\color{blue}\\big)\\color{black}";

            sol3sub += nerdamer(sol2sub).evaluate().toString() + ")";
            sol4sub += nerdamer(sol2sub).evaluate().toString() + ")";
            vtfinalofsubresult5 +=
              "\\color{blue}+\\color{black}\\big(" +
              nerdamer(sol3sub).evaluate().toString() +
              "\\big)";
            sol3sub = "";
            sol2sub = "";
    ...
    if (submatrix.length == 2) {
      document.getElementById(
        "singlematrixexplanation"
      ).innerHTML += vtfinalfirst;
      document.getElementById(
        "singlematrixexplanation"
      ).innerHTML += vtfinalfirstresult;
      document.getElementById(
        "singlematrixexplanation"
      ).innerHTML += vtfinalfirstresult2;
    ...
    if (submatrix.length > 2) {
      ...
      if (submatrixofsub.length == 2) {
        ...
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalresult;
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalresult2;
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalresult3;
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalresult4;
        var dis = "\\[\\begin{vmatrix}";
        for (var om of ma) {
          for (var omm of om) {
            dis += omm + "&";
      ...
      if (submatrixofsub.length > 2) {
        ...
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalofsub;
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalofsubresult;
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalofsubresult2;
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalofsubresult3;
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalofsubresult4;
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalofsubresult5;
        document.getElementById(
          "singlematrixexplanation"
        ).innerHTML += vtfinalofsubresult6;
        var dis = "\\[\\begin{vmatrix}";
        for (var om of ma) {
          for (var omm of om) {
            dis += omm + "&";
...
function inverse() {
  ...
  setTimeout(function () {
    ...
    inverseexplanation +=
      "<div style='border:none;border-radius:30px;margin:2px'>\\[Augmented \\space Matrix\\]";
    inverseexplanation += "\\[\\begin{bmatrix}";
    for (var i = 0; i < row; i++) {
      for (var j = 0; j < 2 * column; j++) {
        if (j < row) inverseexplanation += inversematrix[i][j] + "&";
        else if (j - i == row) {
          inversematrix[i][j] = 1;
          inverseexplanation += inversematrix[i][j] + "&";
        } else {
    ...
    for (var i = 0; i < row; i++) {
      ...
      inverseexplanation +=
        "<div style='border:none;border-radius:30px;margin:2px'>\\[Making \\space non-diagonal \\space elements \\space of \\space column \\space " +
        (i + 1) +
        "\\space as \\space 0 \\]";
      inverseexplanation += "\\[\\begin{bmatrix}";
      for (var l = 0; l < row; l++) {
        for (var m = 0; m < 2 * column; m++) {
          inverseexplanation +=
            parseFloat(inversematrix[l][m]).toFixed(3) + "&";
        }
        inverseexplanation = inverseexplanation.slice(0, -1);
        inverseexplanation += "\\\\";
      }
    ...
    inverseexplanation +=
      "<div style='border:none;border-radius:30px;margin:2px'>\\[Dividing \\space each \\space row \\space by \\space corresponding diagonal \\space element \\space of \\space the \\space Matrix\\]";
    inverseexplanation += "\\[\\begin{bmatrix}";
    for (var i = 0; i < row; i++) {
      for (var j = 0; j < 2 * column; j++) {
    ...
    inverseexplanation +=
      "<div style='border:none;border-radius:30px;margin:2px'>\\[Inverse \\space of \\space the \\space given \\space Matrix\\]";
    inverseexplanation += "\\[\\begin{bmatrix}";
    for (var i = 0; i < row; i++) {
