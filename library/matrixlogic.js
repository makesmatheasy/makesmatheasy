...
function sumofmatrix(matrix1, matrix2, row2, col2) {
    var summ = [];
    var sumexplanation = '';
    for (var i = 0; i < row2; i++) {
        ...
        for (var j = 0; j < col2; j++) {
            ...
            for (var ii = 0; ii < row2; ii++) {
                for (var jj = 0; jj < col2; jj++) {
                    if (ii == i && jj == j) {
                        dp += matrix1[ii][jj] + "&"
                    } else {
                        dp += ".&";
...
//matrix power
function matrixPower(matrixsingle, row, col, pow) {
    var mul = [];
    var matrix2 = [];
    for (var i = 0; i < row; i++) {
        matrix2[i] = [];
    ...
    return { 'result': '', 'steps': mulexplanation };
}

//rank calculation
function rank(matrixsingle, row, column) {
...
function minorsAndCoFactors(matrixsingle, row, col) {
...
function inverse(matrixsingle, row, column) {
    ...
    return { 'result': '', 'steps': inverseexplanation }
}
